...
protected:
  ...
  void leave(std::shared_ptr<ExecutionFrame> frame) {
    ...
    //     else
    //       throw new InternalCompilerError("Call to a non-function
    //       instruction!");
    //   }
    //   else
    //     throw new InternalCompilerError("Invalid function call '" + insn + "'
    //     (target '" + target.value + "' is unknown)!");
    ...
    //   else
    //     throw new InternalCompilerError("Invalid value in CJUMP: expected
    //     0/1, got " + top);
    //   if (label != null)
