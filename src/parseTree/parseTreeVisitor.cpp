...
std::shared_ptr<ast::ProgramDecl> visitProgramDecl(const NodePtr& node) {
    ...
    visitListPattern<nodeType::ImportDeclList, ast::ImportDecl, true>(node->child(1), imports);
...
std::shared_ptr<ast::QualifiedIdentifier> visitPackageDecl(const NodePtr& node) {
...
ast::ImportDecl visit<nodeType::ImportDeclList>(const NodePtr& node) {
    ...
    switch (node->get_node_type()) {
        ...
        default:
            throw std::runtime_error("visit<ImportDeclList> called on an invalid node type");
    }
}
...
std::shared_ptr<ast::ClassDecl> visitClassDecl(const NodePtr& node) {
    ...
    visitListPattern<nodeType::InterfaceTypeList, std::shared_ptr<ast::QualifiedIdentifier>, true>(node->child(3), interfaces);

    // visit Class Body
    std::vector<std::shared_ptr<ast::Decl>> classBodyDecls;
    visitListPattern<nodeType::ClassBodyDeclList, std::shared_ptr<ast::Decl>, true>(node->child(4), classBodyDecls);

    // Return the constructed AST node
    return std::make_shared<ast::ClassDecl>(modifiers, name, super, interfaces, classBodyDecls);
}
...
template <>
std::shared_ptr<ast::QualifiedIdentifier> visit<nodeType::InterfaceTypeList>(const NodePtr& node) {
    return visitQualifiedIdentifier(node);
}

template <>
std::shared_ptr<ast::Decl> visit<nodeType::ClassBodyDeclList>(const NodePtr& node) {
    switch (node->get_node_type()) {
...
std::shared_ptr<ast::MethodDecl> visitMethodDecl(const NodePtr& node) {
    ...
    // type could be void
    std::shared_ptr<ast::Type> type = (node->num_children() == 5) ? visitType(node->child(1)) : nullptr;
    std::string name = visitIdentifier(node->child(type ? 2 : 1));
    
    std::vector<std::shared_ptr<ast::VarDecl>> params;
    // visit params
    visitListPattern<nodeType::FormalParameterList, std::shared_ptr<ast::VarDecl>, true>(node->child(type ? 3 : 2), params);

    // visit body
    std::shared_ptr<ast::Stmt> body = node->child(type ? 4 : 3) ? visitBlock(node->child(type ? 4 : 3)) : nullptr;

    return std::make_shared<ast::MethodDecl>(modifiers, name, type, params, false, body);
}
...
std::shared_ptr<ast::MethodDecl> visitConstructorDecl(const NodePtr& node) {
    ...
    std::vector<std::shared_ptr<ast::VarDecl>> params;
    visitListPattern<nodeType::FormalParameterList, std::shared_ptr<ast::VarDecl>, true>(node->child(2), params);

    // visit body if have one
    std::shared_ptr<ast::Stmt> body = node->child(3) ? visitBlock(node->child(3)) : nullptr;
    
    return std::make_shared<ast::MethodDecl>(modifiers, name, nullptr, params, true, body);
}

// FIXME: we need formal parameter that strictly have 2 children!
template <>
std::shared_ptr<ast::VarDecl> visit<nodeType::FormalParameterList>(const NodePtr& node) {
...
std::shared_ptr<ast::InterfaceDecl> visitInterfaceDecl(const NodePtr& node) {
    ...
    visitListPattern<nodeType::InterfaceTypeList, std::shared_ptr<ast::QualifiedIdentifier>, true>(node->child(2), extends);
    
    std::vector<std::shared_ptr<ast::Decl>> interfaceBodyDecls;
    visitListPattern<nodeType::InterfaceBodyDeclList, std::shared_ptr<ast::Decl>, true>(node->child(3), interfaceBodyDecls);
    
    return std::make_shared<ast::InterfaceDecl>(modifiers, name, extends, interfaceBodyDecls);
}
...
std::shared_ptr<ast::MethodDecl> visitAbstractMethodDecl(const NodePtr& node) {
    ...
    return std::make_shared<ast::MethodDecl>(modifiers, name, type, params, false, nullptr);
}

template <>
std::shared_ptr<ast::Decl> visit<nodeType::InterfaceMemberDeclList>(const NodePtr& node) {
...
std::shared_ptr<ast::Stmt> visitBlock(const NodePtr& node) {
   ...
   check_num_children(node, 1, 1);
   if(node->child(0) == nullptr) return make_shared<ast::Block>();
   // Unimplemented yet
...
std::shared_ptr<ast::QualifiedIdentifier> visitQualifiedIdentifier(
    const NodePtr& node, std::shared_ptr<ast::QualifiedIdentifier> ast_node = nullptr) {
...
ast::Modifiers visitModifierList(const NodePtr& node, ast::Modifiers modifiers = {}) {
    if (!node) {
        return modifiers;
    }
...
std::shared_ptr<ast::Type> visitType(const NodePtr& node) {
    ...
    if (elemType->get_node_type() == nodeType::BasicType) {
        elemType = std::make_shared<ast::BuiltInType>(std::dynamic_pointer_cast<BasicType>(elemType)->get_type());
    } else if (elemType->get_node_type() == nodeType::QualifiedName) {
        elemType = std::make_shared<ast::ReferenceType>(visitQualifiedIdentifier(elemType));
    } else {
        throw std::runtime_error("Expected a BasicType or QualifiedIdentifier node but got " + elemType->type_string());
    }
...
VariableDecl visitVariableDeclarator(const NodePtr& typeNode, const NodePtr& declNode) {
